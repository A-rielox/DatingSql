...
//                                                  NO IMPLEMENTADO
public class LikesPagedList : List<AppUser>
{
    public LikesPagedList(IEnumerable<AppUser> items, int count,
    ...
    // source es la query
    public static async Task<LikesPagedList> CreateAsync(IDbConnection db, int pageNumber, int pageSize,
                                                             ...
                                                             int minAge, int maxAge, string orderBy)
    {
        ...
        return new LikesPagedList(users, count.SingleOrDefault(), pageNumber, pageSize);
    }
}
